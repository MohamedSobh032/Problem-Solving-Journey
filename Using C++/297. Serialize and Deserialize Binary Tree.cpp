...
public:
    string serialize(TreeNode *root)
    {
        ...
        while (!q.empty())
        {
            auto node = q.front();
            q.pop();
            ser.append((node ? std::to_string(node->val) : "null"));
            ser.append(",");
            if (!node)
                continue;
            q.push(node->left);
    ...
    TreeNode *deserialize(string data)
    {
        ...
        for (int i = 0; i < n; i++)
        {
            if (!nodes[i])
                continue;
            nodes[i]->left = (next_to_point < n) ? nodes[next_to_point] : nullptr;
            next_to_point++;
            nodes[i]->right = (next_to_point < n) ? nodes[next_to_point] : nullptr;
            next_to_point++;
        }
        return nodes[0];
    }
...
public:
    void serialize_dfs(TreeNode *root, std::string &stream)
    {
        ...
        stream.append(std::to_string(root->val));
        stream.append(",");

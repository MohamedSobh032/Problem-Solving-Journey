...
public:
    ...
    void rec(const std::vector<std::vector<int>>& matrix, const int& n, const int& m,
        ...
        if (x < 0 || x >= n || y < 0 || y >= m || matrix[x][y] <= last_elem) return;
        curr_len++; max_len = std::max(max_len, curr_len);
        for (const auto dir: dirs) rec(matrix, n, m, matrix[x][y], x + dir.first, y + dir.second, curr_len, max_len);
    }
...
public:
    ...
    int rec(const std::vector<std::vector<int>>& matrix, std::vector<std::vector<int>>& dp,
        ...
        for (const auto& dir: dirs) {
            ...
            if (newx < 0 || newx >= n || newy < 0 || newy >= m || matrix[x][y] >= matrix[newx][newy]) continue;
        ...
        for (int i = 0; i < n; i++) {
            for (int j = 0; j < m; j++) {
                for (const auto& dir : dirs) {
                    ...
                    if (newx >= 0 && newx < n && newy >= 0 && newy < m && matrix[i][j] < matrix[newx][newy]) indegree[newx][newy]++;
                }
            }
        }
            ...
            for (int i = 0; i < size; i++) {
                ...
                for (const auto& dir : dirs) {
                    ...
                    if (newx >= 0 && newx < n && newy >= 0 && newy < m && matrix[x][y] < matrix[newx][newy]) {
                        indegree[newx][newy]--;
                        if (indegree[newx][newy] == 0) q.push({newx, newy});
                    }
                }
            }
        }

...
public:
    ...
    void dfs(const std::vector<std::vector<int>>& heights, std::vector<std::vector<bool>>& isVisited,
        ...
        isVisited[x][y] = true;
        if (x == 0 || y == 0) canReach.first = true;
        if (x == n - 1 || y == m - 1) canReach.second = true;

        for (const auto& dir: dirs) {
            if (canReach.first && canReach.second) return;
            int nx = x + dir.first, ny = y + dir.second;
            if (nx < 0 || nx >= n || ny < 0 || ny >= m || isVisited[nx][ny] || heights[x][y] < heights[nx][ny]) continue;
    ...
    vector<vector<int>> pacificAtlantic(vector<vector<int>>& heights) {
        ...
        n = heights.size(); m = heights[0].size(); std::vector<std::vector<int>> ret;
        for (int x = 0; x < n; x++) {
            for (int y = 0; y < m; y++) {
                ...
                if (canReach.first && canReach.second) ret.push_back({x, y});
            }
        }
        return ret;
    }
...
class Solution {
public:
    std::vector<std::pair<int, int>> dirs = {{1, 0}, {-1, 0}, {0, 1}, {0, -1}}; int n, m;
    ...
    std::vector<std::vector<int>> pacificAtlantic(std::vector<std::vector<int>>& heights) {
        ...
        n = heights.size(); m = heights[0].size(); std::vector<std::vector<int>> result;
        ...
        for (int i = 0; i < n; i++) dfs(heights, reach_pacific, i, 0);
        for (int i = 0; i < m; i++) dfs(heights, reach_pacific, 0, i);
        for (int i = n - 1; i >= 0; i--) dfs(heights, reach_atlantic, i, m - 1);
        for (int i = m - 1; i >= 0; i--) dfs(heights, reach_atlantic, n - 1, i);

...
public:
    vector<int> spiralOrder(vector<vector<int>>& matrix) {
        ...
        while (left < right && top < bottom) {
            ...
            for (int i = top; i < bottom; i++) spiral.push_back(matrix[i][right - 1]);
            right--;

            if (spiral.size() == m * n) break;

            for (int i = right - 1; i >= left; i--) spiral.push_back(matrix[bottom - 1][i]);
            bottom--;

            for (int i = bottom - 1; i >= top; i--) spiral.push_back(matrix[i][left]);
            left++;
        }
        return spiral;
    }

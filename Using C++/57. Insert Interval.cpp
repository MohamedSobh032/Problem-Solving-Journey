...
public:
    vector<vector<int>> insert(vector<vector<int>>& intervals, vector<int>& newInterval) {
        ...
        int n = intervals.size(); std::vector<std::vector<int>> merged;
        for (int i = 0; i < n; i++) {
            if (newInterval[1] < intervals[i][0]) {
                merged.emplace_back(newInterval);
                for (int j = i; j < n; j++) merged.emplace_back(intervals[j]);
                return merged;
            } else if (newInterval[0] > intervals[i][1]) merged.emplace_back(intervals[i]);
            else {
